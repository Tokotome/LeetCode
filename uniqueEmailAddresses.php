...
// It is possible to use both of these rules at the same time. Given an array of strings emails where we send one email to each emails[i], 
...
class Solution {
    ...
    function numUniqueEmails($emails) {
        foreach($emails as &$email) {
            ...
            //ingore everything after the plus sign
            $position = strpos($localName, '+');
            
            if ($position !== false) {
                $localName = substr($localName, 0, $position);
            }
